...
class C_VAE_NET(object):
    ...
    def build_vae(self, input_data, conditions=None):
        ...
        # >>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>
        # CHANGED: use dynamic batch size everywhere instead of self.batch_size
        batch = tf.shape(input_data)[0]  # dynamic batch
        # <<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<
        ...
        # CHANGED: use dynamic batch
        enc_state = self.cell_enc.zero_state(batch, tf.float32)   # CHANGED
        dec_state = self.cell_dec.zero_state(batch, tf.float32)   # CHANGED

        # CHANGED: random noise with dynamic batch
        self.e = tf.random_normal((batch, self.z_dim))            # CHANGED

        # placeholders for time steps
        ...
        for t in range(self.time_steps):
            # CHANGED: dynamic zeros
            c_prev = tf.zeros((batch, self.dim)) if t == 0 else self.c[t - 1]  # CHANGED
            c_sigmoid = tf.sigmoid(c_prev)

            # CHANGED: safe slice for x_t (remove reshape); take only the first dim cols (exclude cond labels)
            if self.conditional:
                x_input_t = tf.unstack(input_data, axis=1)[t][:, :self.dim]    # CHANGED
            else:
                x_input_t = input_enc[t][:, :self.dim]                         # CHANGED

            x_hat = x_input_t - c_sigmoid                                      # CHANGED
            ...
            # cVAE
            if self.conditional:
                z_t  = mu[t] + sigma[t] * self.e
                z[t] = tf.concat([z_t, conditions], axis=-1)
            else:
    ...
    def reconstruct_decoder(self, dec_input, conditions=None):
        # dec_input is a list/stack unstacked outside. Use dynamic batch from the first element.
        batch = tf.shape(dec_input[0])[0]  # CHANGED

        rec_decoded = [0] * self.time_steps
        rec_dec_state = self.cell_dec.zero_state(batch, dtype=tf.float32)  # CHANGED
        for t in range(self.time_steps):
            if self.conditional:
    ...
    def weight_variable(self, shape, scope_name, name):
        with tf.variable_scope(scope_name, reuse=tf.AUTO_REUSE):
            wv = tf.get_variable(name=name, shape=shape, initializer=tf.glorot_uniform_initializer())
        return wv

    def bias_variable(self, shape, scope_name, name=None):
        with tf.variable_scope(scope_name, reuse=tf.AUTO_REUSE):
            bv = tf.get_variable(name=name, shape=shape, initializer=tf.glorot_uniform_initializer())
        return bv
...
class D_VAE_NET(object):
    ...
    def build_vae(self, input_data, conditions=None):
        ...
        # >>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>
        # CHANGED: dynamic batch
        batch = tf.shape(input_data)[0]  # CHANGED
        # <<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<
        ...
        for t in range(self.time_steps):
            ...
            # CHANGED: slice without reshape
            if self.conditional:
                x_input_t = tf.unstack(input_data, axis=1)[t][:, :self.dim]
            else:
                x_input_t = input_enc[t][:, :self.dim]

            x_hat = x_input_t - c_sigmoid
...
class C_GAN_NET(object):
    ...
    def build_GenRNN(self, input_noise, conditions=None):
        ...
        # create initial states for multi-cell lstm (still fine to use fixed batch here)
        initial_state = []
        for i in range(self.gen_num_layers):
