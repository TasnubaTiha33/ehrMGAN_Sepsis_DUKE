...
class m3gan(object):
    ...
    def build_tf_graph(self):
        ...
        # Pretrain vae for c
        if self.conditional:
            self.real_data_label_pl = tf.placeholder(              # CHANGED
                dtype=float, shape=[None, self.num_labels], name="real_data_label")

        self.c_real_data_pl = tf.placeholder(
            dtype=float, shape=[None, self.time_steps, self.c_dim], name="continuous_real_data")  # CHANGED
        ...
        self.c_vae_test_data_pl = tf.placeholder(
            dtype=float, shape=[None, self.time_steps, self.c_dim], name="vae_validation_c_data")
        if self.conditional:
        ...
        self.d_real_data_pl = tf.placeholder(
            dtype=float, shape=[None, self.time_steps, self.d_dim], name="discrete_real_data")  # CHANGED
        if self.conditional:
        ...
        self.d_vae_test_data_pl = tf.placeholder(
            dtype=float, shape=[None, self.time_steps, self.d_dim], name="vae_validation_d_data")
        if self.conditional:
        ...
        # dgan - discriminator
        self.d_fake = self.dgan.build_Discriminator(self.d_gen_decoded)      # returns only logits in our updated networks
    ...
    def build_loss(self):
        ...
        # 3. Contrastive loss
        ### CHANGED: use dynamic batch for reshape parameters (safer with TF1 compat).
        batch_dyn = tf.shape(x_latent_1)[0]                # CHANGED
        self.vae_contra_loss = nt_xent_loss(               # CHANGED
            tf.reshape(x_latent_1, [batch_dyn, -1]),
            tf.reshape(x_latent_2, [batch_dyn, -1]),
            batch_dyn)
        ...
        if self.conditional:
            ...
            with tf.variable_scope("Shared_VAE/semantic_classifier"):
                ...
                vae_logits = tf.layers.dense(vae_hidden_layer, units=self.num_labels, activation=tf.nn.tanh)  # CHANGED units
        ...
        ###########################
        # NOTE: ones_target / zeros_target are written to expect fixed batch. They should still work as long as
        #       you feed self.batch_size for training. If you do dynamic batch here, adapt those utils as well.
    ...
    def train(self):
        ...
        for pre in range(self.num_pre_epochs):
            ...
            # prepare data (same index for all)
            ...
            val_batch_size = min(self.batch_size, continuous_x_test.shape[0])
            continuous_x_test_batch = continuous_x_test[random_idx_[:val_batch_size]]
            discrete_x_test_batch = discrete_x_test[random_idx_[:val_batch_size]]
            ...
            for b in range(num_batches):
                ...
                if ((pre + 1) % self.epoch_loss_freq == 0 or pre == self.num_pre_epochs - 1):
                    summary_result, loss_val = self.sess.run([self.c_vae_summary, self.c_vae_loss], feed_dict=feed_dict)
                    print(f"[Pretrain C-VAE] Epoch {pre + 1}, Batch {b + 1}/{num_batches}, Loss: {loss_val:.4f}")
                    self.summary_writer.add_summary(summary_result, global_id)

                _ = self.sess.run(self.d_v_op_pre, feed_dict=feed_dict)
                if ((pre + 1) % self.epoch_loss_freq == 0 or pre == self.num_pre_epochs - 1):
                    summary_result, loss_val = self.sess.run([self.d_vae_summary, self.d_vae_loss], feed_dict=feed_dict)
                    print(f"[Pretrain D-VAE] Epoch {pre + 1}, Batch {b + 1}/{num_batches}, Loss: {loss_val:.4f}")
                    self.summary_writer.add_summary(summary_result, global_id)

                # (Optional) collect reconstructions if needed
                # c_recon, d_recon = self.sess.run([self.c_decoded_output, self.d_decoded_output], feed_dict=feed_dict)
                # c_real_data_lst.append(feed_dict[self.c_real_data_pl])
                # c_rec_data_lst.append(c_recon)
                # d_real_data_lst.append(feed_dict[self.d_real_data_pl])
                # d_rec_data_lst.append(d_recon)

                global_id += 1

            if ((pre + 1) % self.epoch_ckpt_freq == 0 or pre == self.num_pre_epochs - 1):
                data_gen_path = os.path.join("data/fake/", "pretrain_epoch{}".format(pre))
                ...
                # 🔁 REPLACEMENT FOR MISSING visualise_gan
                print(f"[Pretrain] Epoch {pre + 1}: Skipping GAN visualization (visualise.py not found)")  # CHANGED

        # (Optional) save recon pairs; guard against empty lists
        # if c_real_data_lst:
        #     np.savez('data/fake/vae.npz',
        #              c_real=np.vstack(c_real_data_lst), c_rec=np.vstack(c_rec_data_lst),
        ...
        for e in range(self.num_epochs):
            ...
            for b in range(num_batches):
                ...
                # feed c
                feed_dict[self.c_real_data_pl] = continuous_x_random[b * self.batch_size: (b + 1) * self.batch_size]
                feed_dict[self.c_gen_input_noise_pl] = self.gen_input_noise(self.batch_size, self.time_steps,
                                                                            noise_dim=self.c_noise_dim)
                feed_dict[self.c_vae_test_data_pl] = continuous_x_test_batch
                # feed d
                feed_dict[self.d_real_data_pl] = discrete_x_random[b * self.batch_size: (b + 1) * self.batch_size]
                feed_dict[self.d_gen_input_noise_pl] = self.gen_input_noise(self.batch_size, self.time_steps,
                                                                            noise_dim=self.d_noise_dim)
                feed_dict[self.d_vae_test_data_pl] = discrete_x_test_batch
                ...
                for _ in range(self.v_rounds):
                    ...
                    if ((e + 1) % self.epoch_loss_freq == 0 or e == self.num_epochs - 1):
                        summary_result = self.sess.run(self.d_vae_summary, feed_dict=feed_dict)  # CHANGED: use c_vae_summary
                        self.summary_writer.add_summary(summary_result, global_id)
            ...
            if ((e + 1) % self.epoch_ckpt_freq == 0 or e == self.num_epochs - 1):
                ...
                print(f"[Joint] Epoch {e + 1}: Skipping GAN visualization (visualise.py not found)")  # CHANGED
    ...
    def generate_data(self, num_sample, labels=None):
        ...
        c_gen_data = []
        round_ = num_sample // self.batch_size
        remainder = num_sample % self.batch_size

        for i in range(round_):
            d_gen_input_noise = self.gen_input_noise(self.batch_size, self.time_steps, self.d_noise_dim)
            c_gen_input_noise = self.gen_input_noise(self.batch_size, self.time_steps, self.c_noise_dim)

            feed_dict = {
                self.d_gen_input_noise_pl: d_gen_input_noise,
                self.c_gen_input_noise_pl: c_gen_input_noise
            }

            if self.conditional and labels is not None:
                feed_dict[self.real_data_label_pl] = labels[i * self.batch_size: (i + 1) * self.batch_size]
        ...
        if remainder > 0:
            ...
            if self.conditional and labels is not None:
                label_batch = labels[round_ * self.batch_size:]
                pad_labels = np.zeros((pad_len, self.num_labels), dtype=np.float32)
                padded_labels = np.concatenate([label_batch, pad_labels], axis=0)
                feed_dict[self.real_data_label_pl] = padded_labels

            d_gen_data_, c_gen_data_ = self.sess.run([self.d_gen_decoded, self.c_gen_decoded], feed_dict=feed_dict)
            d_gen_data_ = np.stack(d_gen_data_, axis=1)[:remainder]
            c_gen_data_ = np.stack(c_gen_data_, axis=1)[:remainder]

            d_gen_data.append(d_gen_data_)
            c_gen_data.append(c_gen_data_)
